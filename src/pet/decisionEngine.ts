...
export function rankActions(state: PetState, context: Context, random: () => number = Math.random, traits: Personality = personality): ScoredAction[] {
  const s = state.stats;
...
export function decide(state: PetState, context: Context, random: () => number = Math.random): Action {
  const best = rankActions(state, context, random)[0];
