...
export interface PetState { version: 1; stats: Stats; relationship: Relationship; memory: Memory; updatedAt: string }
export function createState(now = new Date().toISOString()): PetState {
  return { version: 1, stats: { ...initialStats }, relationship: { ...initialRelationship }, memory: createMemory(now), updatedAt: now };
}
